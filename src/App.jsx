...
const handleRegister = async (e) => {
  ...
  setIsLoading(true);
  try {
    const response = await fetch('https://vending-api-server.onrender.com/api/registro', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nombre: 'Nuevo Cliente', email, password })
    });
    const data = await response.json();
    if (data.success) {
...
// En tu lógica de vista (if view === 'login'):
if (view === 'login') {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md">
        <h2 className="text-3xl font-bold text-center mb-8 text-gray-800">
          {isRegistering ? 'Crear Cuenta' : 'Smart Vending SaaS'}
        </h2>
        <form className="space-y-5" onSubmit={isRegistering ? handleRegister : handleLogin}>
          ...
          <div>
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-2.5 border rounded-xl" required />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Contraseña</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-2.5 border rounded-xl" required />
          </div>
          ...
          <p className="text-center text-sm mt-4 text-gray-600">
            ...
            <button type="button" onClick={() => setIsRegistering(!isRegistering)} className="text-blue-600 font-bold ml-1">
